...
def log_eval_metrics(logger, log_prefix, eval_dir, num_eval_eps):
    ...
    multi_task_data = [pickle.load(open(
        str(eval_dir)+'/eval_ep_'+str(idx)+'.pkl', 'rb')) for idx in range(num_eval_eps)]
    keys = multi_task_data[0].keys()
    for key in keys:
        for idx in range(num_eval_eps):
            logger.scalar(log_prefix + 'task_'+str(idx) +
                          '/'+key, multi_task_data[idx][key])

        _avg = np.mean([multi_task_data[idx][key]
                       for idx in range(num_eval_eps)])
...
def sample_episodes(episodes, length=None, balance=False, seed=0):
    ...
    while True:
        ...
        if length:
            ...
            if available < 1:
                print(
                    f'Skipped short episode of length {total}, need length {length}.')
                continue
            if balance:
...
class OneHotDist(tfd.OneHotCategorical):
    ...
    def sample(self, sample_shape=(), seed=None):
        ...
        sample = tf.cast(super().sample(
            sample_shape, seed), self._sample_dtype)
...
class Optimizer(tf.Module):
    ...
    def __call__(self, tape, loss, modules):
        ...
        # if self._clip:
        #     grads, _ = tf.clip_by_global_norm(grads, self._clip)
...
def am_sampling(obs, actions):
    ...
    for traj_idx in range(num_trajs):
        ...
        # positive
        s_t.append(tf.concat([obs[traj_idx, end_idx-1],
                   obs[traj_idx, end_idx]], axis=-1))
        s_tp1.append(tf.concat([obs[traj_idx, end_idx],
                     obs[traj_idx, end_idx]], axis=-1))
        ...
        s_t.append(tf.concat([obs[traj_idx, end_idx-1],
                   obs[neg_traj_idx, neg_seq_idx]], axis=-1))
        s_tp1.append(tf.concat([obs[traj_idx, end_idx],
                     obs[neg_traj_idx, neg_seq_idx]], axis=-1))
        a_t.append(actions[traj_idx, end_idx])
        r_t.append(0)
...
def get_future_goal_idxs(seq_len, bs):
    ...
    for cur_idx in range(seq_len):
        for goal_idx in range(cur_idx, seq_len):
            cur_idx_list.append(np.concatenate(
                [np.ones((bs, 1), dtype=np.int32)*cur_idx, np.arange(bs).reshape(-1, 1)], axis=-1))
            goal_idx_list.append(np.concatenate(
                [np.ones((bs, 1), dtype=np.int32)*goal_idx, np.arange(bs).reshape(-1, 1)], axis=-1))
...
def get_future_goal_idxs_neg_sampling(num_negs, seq_len, bs, batch_len):
    ...
    for i in range(num_negs):
        goal_idxs[i, 1] = np.random.choice(
            [j for j in range(bs) if j//batch_len != cur_idxs[i, 1]//batch_len])
        ...
        for i in range(num_batches):
            for j in range(seq_len):
                if relabel_mode == 'uniform':
                    next_obs_idxs_for_relabelling.append(
                        np.random.randint((seq_len*i)+j, seq_len * (i + 1)))

                elif relabel_mode == 'geometric':
                    next_obs_idxs_for_relabelling.append(
                        min((seq_len * i) + j + deltas[i, j] - 1, seq_len * (i + 1) - 1))

                relabel_masks.append(
                    int(next_obs_idxs_for_relabelling[-1] != ((seq_len*i) + j)))

        next_obs_idxs_for_relabelling = np.array(
            next_obs_idxs_for_relabelling).reshape(-1, 1)
        relabel_masks = np.array(
            relabel_masks, dtype=np.float16).reshape(-1, 1)

        idxs = np.random.permutation(num_points).reshape(-1, 1)
        relabel_idxs, non_relabel_idxs = np.split(
            idxs, (int(relabel_fraction*num_points),), axis=0)
        curr_state, action, next_state = tf.gather_nd(curr_state, idxs), tf.gather_nd(
            action, idxs),  tf.gather_nd(next_state, idxs)

        _relabeled_goals = tf.gather_nd(
            next_state, next_obs_idxs_for_relabelling[relabel_idxs.flatten()])
        if feat_to_embed_func is not None:
            _relabeled_goals = feat_to_embed_func(_relabeled_goals)

        _goals = tf.concat(
            [_relabeled_goals, tf.gather_nd(_goals, non_relabel_idxs)], axis=0)
        masks = tf.concat([tf.squeeze(relabel_masks[relabel_idxs.flatten()]), tf.gather_nd(
            masks, non_relabel_idxs)], axis=0)
