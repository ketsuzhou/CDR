...
class GCWorldModel(models.WorldModel):
  ...
  def train(self, data):
    ...
    with tf.GradientTape() as model_tape:
        ...
        #     tf.cast(post['cd_deter_mean'], tf.float32), tf.cast(post['cd_deter_std'], tf.float32))
        # cd_deter_like = dist.log_prob(
        #     tf.stop_gradient(tf.cast(post['deter'], tf.float32)))
        # cd_likes = tf.reduce_mean(cd_deter_like)
        cd_likes = tf.reduce_mean(
            tf.abs(tf.cast(post['cd_deter_mean'], tf.float32)-tf.stop_gradient(tf.cast(post['deter'], tf.float32))))
        
        model_loss = kl_loss - sum(likes.values()) - cd_likes
