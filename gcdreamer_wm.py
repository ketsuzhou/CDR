...
class GCWorldModel(models.WorldModel):
    ...
    def __init__(self, step, config):
        ...
        if config.remove_recur_info:
            self.heads['pred_init_post_stoch'] = networks.DenseHead(
                [self._config.dyn_stoch], 
                2, config.units, config.act, std='learned')
        ...
        if self._config.gc_input == 'skills':
            self.rev_pred = networks.get_mlp_model(
                'feat_rp', [256, 128], self._config.skill_dim)
            if self._config.double_rev_pred:
                self.embed_rev_pred = networks.get_mlp_model(
                    'embed_rp', [256, 128], self._config.skill_dim)

    def train(self, data):
        data = self.preprocess(data)
        with tf.GradientTape() as model_tape:
            embed = self.encoder(data)
            data['embed'] = tf.stop_gradient(
                embed)  # Needed for the embed head

        if 'pred_init_post_stoch' in [name for name, _ in self.heads.items()]:
            # self.get_init_feat_embed(data['embed'][0])
            data['pred_init_post_stoch'] = tf.stop_gradient(
                tf.vectorized_map(self.get_init_post_stoch, data['embed']))

        with model_tape:
            self.dynamics.causal_transition.is_training = True
            post, prior = self.dynamics.observe(embed, data['action'])
            data['stoch_state'] = tf.stop_gradient(
                post['stoch'])  # Needed for the embed head
            ...
            for name, head in self.heads.items():
                ...
                data_ = data[name]
                # if name == 'image':
                #     select_embed = embed[0]
                #     select_image = data_[0]
                #     init_feat = self.get_init_feat_embed(select_embed)
                #     # init_feat = tf.vectorized_map(self.get_init_feat_embed, select_embed)
                #     inp = tf.concat([init_feat[None], inp], 0)
                #     data_ = tf.concat([select_image[None], data_], 0)

                if name == 'pred_init_post_stoch':
                    inp = post['deter']
                    # data['pred_init_feat_embed'] = data['pred_init_feat_embed'] 
                if name == 'reward':
                ...
                # tf.reduce_mean(pred._mean() - data[name])
                like = pred.log_prob(tf.cast(data_, tf.float32))
                likes[name] = tf.reduce_mean(
                    like) * self._scales.get(name, 1.0)
                # if name == 'image':
                #     like_init_post_stoch = tf.reduce_mean(like[0])
            model_loss = kl_loss - sum(likes.values())
            self.dynamics.causal_transition.is_training = False
            if self._config.latent_constraint == 'consecutive_state_l2':
                seq_len = feat.shape[1]
                for i in range(seq_len-1):
                    model_loss += 0.1 * \
                        tf.cast(tf.reduce_mean(
                            (feat[:, i, :] - feat[:, i+1, :])**2), tf.float32)
            ...
            if self._config.gc_input == 'skills' and self._config.double_rev_pred:
                ...
                skill_loss = tf.reduce_mean(
                    (self.embed_rev_pred(embed) - skill_target) ** 2)
        ...
        # metrics['like_init_post_stoch'] = like_init_post_stoch
    ...
    def get_goal(self, obs, training=False):
        if self._config.gc_input == 'state':
            ...
            if (not training) or self._config.training_goals == 'env':
                ...
                _embed = self.encoder(
                    {'image': obs['image_goal'], 'state': obs.get('goal', None)})
                ...
                if self._config.labelled_env_multiplexing:
                    ...
                    # rows are samples, columns are envs
                    oh_ids = tf.one_hot(env_ids, l)
                    # co-occurence matrix, mat_ij = 0 <==> i,j are the same env
                    cooc = tf.gather(oh_ids, env_ids, axis=1)
                    # Sample from the same env
                    ids = tf.random.categorical(tf.math.log(
                        cooc / tf.reduce_sum(cooc, 1, keepdims=True)), 1)[:, 0]
