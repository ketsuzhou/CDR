...
class WorldModel(tools.Module):
    ...
    def __init__(self, step, config):
        ...
        self.tran_dynamics = config.tran_dynamics
        dynamics = dict(
            RSSM=networks.RSSM,
            TSSM=networks.TSSM)[config.tran_dynamics]
    ...
    def preprocess(self, obs):
        ...
        if self._config.clip_rewards[0] == 'd':
            ...
            def r_transform(r): return r / float(self._config.clip_rewards[1:])
        else:
    ...
    def get_init_feat(self, obs, state=None, sample=False):
        ...
        latent, _ = self.dynamics.obs_step(
            latent, action, embed, sample=sample, first_state=first_state)
    ...
    def get_init_post_stoch(self, embed, sample=False):
        latent = self.get_init_state_embed(embed, sample)
        return latent['stoch']

    def get_im_embeding(self, embed, sample=False):

        stats = self.dynamics._suff_stats_layer(
            self.dynamics.post2stoch(embed))
        # stats = self._suff_stats_layer(self.pri2stoch(x))

        if sample:
            stoch = self.dynamics.get_dist(stats).sample()
        else:
            stoch = self.dynamics.get_dist(stats).mode()
    ...
    def get_init_state_embed(self, embed, sample=False):
        ...
        action = tf.zeros((batch_size, self._config.num_actions), dtype)
        latent, _ = self.dynamics.obs_step(
            latent, action, embed, first_state=True, sample=sample)
        return latent
...
class ImagBehavior(tools.Module):
    ...
    def train(
        ...
        with tf.GradientTape() as value_tape:
            ...
            # value_loss = tf.abs(value - tf.stop_gradient(target))
    ...
    def _imagine(self, start, policy, horizon, repeats=None):
        ...
        def step(prev, step):
            ...
            succ = dynamics.img_step(
                state, action, first_state=step==0, sample=self._config.imag_sample)
    ...
    def _compute_actor_loss(
            ...
            weights, imag_gradient=None):
        if imag_gradient == None:
            imag_gradient = self._config.imag_gradient
        ...
        if imag_gradient == 'dynamics':
            ...
            raise NotImplementedError(imag_gradient)
...
class CausalImagBehavior(ImagBehavior):
    def __init__(self, config, world_model, stop_grad_actor=True, reward=None):
        super().__init__(config, world_model, stop_grad_actor=stop_grad_actor, reward=reward)

        # self.cond_entropy, self.slow_cond_entropy = (
        #     networks.DenseHead(
        #         [], config.reward_layers, config.units, config.act, stcoh=False)
        #     for _ in range(2))

        # self.cond_entropy_updates = tf.Variable(0, trainable=False)

        self.expl_updates = tf.Variable(0, trainable=False)
        self.expl_slow_updates = config.expl_slow_updates
        self.expl_fraction = config.expl_fraction

        kw = dict(wd=config.weight_decay, opt=config.opt)
        self._actor_opt = tools.Optimizer(
            'actor', config.expl_actor_lr, config.opt_eps, config.actor_grad_clip, **kw)
        self._value_opt = tools.Optimizer(
            'value', config.expl_value_lr, config.opt_eps, config.value_grad_clip, **kw)

        self.value = networks.DenseHead(
            [], 5, config.units, config.act,
            config.value_head)
    ...
    @tf.function
    def train(self, start, objective=None, imagine=None, tape=None, repeats=None):
        objective = objective or self._reward
        ...
        #     assert bool(objective) != bool(imagine)
        imag_feat, imag_state, imag_action = self._imagine(
            start, self.actor, 12, repeats)

        reward = objective(imag_feat, imag_state, imag_action)
        imag_feat = imag_feat[:-1]
        imag_action = imag_action[:-1]
        target, weights = self._compute_target(
            imag_feat, reward, self._config.slow_actor_target)
        with (tape or tf.GradientTape()) as actor_tape:
            assert bool(objective) != bool(imagine)
            actor_loss, mets = self._compute_actor_loss(
                imag_feat, imag_action, target, weights)
            metrics.update(mets)
        metrics.update(self._actor_opt(actor_tape, actor_loss, [self.actor]))
    ...
    def _compute_actor_loss(
            self, imag_feat, imag_action, target, weights):
    ...
    @tf.function
    def _update_slow_value(self):
        if self.expl_updates % self.expl_slow_updates == 0:
            for s, d in zip(self.value.variables, self._slow_value.variables):
                d.assign(self.expl_fraction * s + (1 - self.expl_fraction) * d)
        self.expl_updates.assign_add(1)
