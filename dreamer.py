...
class Dreamer(tools.Module):
    def __init__(self, config, logger, dataset):
        ...
        with tf.device('cpu:0'):
            self._step = tf.Variable(count_steps(
                config.traindir), dtype=tf.int64)
        # Schedules.
        ...
        self._task_behavior = dict(
            dreamer=models.ImagBehavior(
                config, self._wm, config.behavior_stop_grad),
            gcdreamer=gcdreamer_imag.GCDreamerBehavior(
                config, self._wm, config.behavior_stop_grad),
        )[config.task_behavior]
        def reward(f, s, a): return self._wm.heads['reward'](f).mode()
        self._expl_behavior = dict(
            greedy=lambda: self._task_behavior,
            random=lambda: expl.Random(config),
            plan2explore=lambda: expl.Plan2Explore(config, self._wm, reward),
            causal_plan2explore=lambda: expl.Causal_Plan2Explore(config, self._wm, reward),
    ...
    def _policy(self, obs, state, training, reset, should_expl=False):
        ...
        if not training:
            ...
            if self._config.offpolicy_opt:
                if self._config.offpolicy_use_embed:
                    action = self._off_policy_handler.actor(
                        tf.concat([self._wm.encoder(obs), goal], axis=-1))
                else:
                    action = self._off_policy_handler.actor(
                        tf.concat([obs['image'], obs['image_goal']], axis=-1))
                ...
                def pad(x): return tf.expand_dims(x, 0)
                reward = self._task_behavior._gc_reward(
                    pad(actor_inp), latent, action, pad(obs['image_goal']))
            ...
            if self._config.offpolicy_use_embed:
                action = self._off_policy_handler.actor(tf.concat([self._wm.encoder(
                    obs), self._wm.encoder({'image': obs['image_goal']})], axis=-1))
            else:
                action = self._off_policy_handler.actor(
                    tf.concat([obs['image'], obs['image_goal']], axis=-1))
        else:
        ...
        if self._config.actor_dist == 'onehot_gumble':
            action = tf.cast(
                tf.one_hot(tf.argmax(action, axis=-1),
    ...
    def _train(self, data):
        ...
        metrics.update(mets)
        start = post
        # ind = tf.range(feat.shape[1], delta=2)
        # aa = {name: tf.gather(post[name], ind, axis=1) for name in post}
        ...
        if self._config.gc_reward == 'dynamical_distance' and self._config.dd_train_off_policy:
            metrics.update(self._task_behavior.train_dd_off_policy(
                self._wm.encoder(self._wm.preprocess(data))))

        if self._config.expl_behavior != 'greedy':
            mets = self._expl_behavior.train(start, feat, embed, kl, data['action'])[-1]
            metrics.update(
                {'expl_' + key: value for key, value in mets.items()})

        if self._config.gcbc:
            _data = self._wm.preprocess(data)
            obs = self._wm.encoder(self._wm.preprocess(
                data)) if self._config.offpolicy_use_embed else _data['image']
            metrics.update(self._off_policy_handler.train_gcbc(
                obs, _data['action']))
...
def make_dataset(episodes, config):
    ...
    def generator(): return tools.sample_episodes(
...
def make_env(config, logger, mode, train_eps, eval_eps, use_goal_idx=False, log_per_goal=False):
    ...
    if 'dmc' in config.task:
        ...
        env = envs.DmcEnv(task, config.size,
        ...
        kitchen_env = envs.KitchenEnv(
            config.action_repeat, use_goal_idx, False)
        robobin_env = envs.RoboBinEnv(
            config.action_repeat, use_goal_idx, False)
        dmc_envs = list(envs.DmcEnv(task, config.size, config.action_repeat,
                        use_goal_idx, log_per_goal) for task in ['walker_walk', 'quadruped_run'])

        env = envs.MultiplexedEnv([kitchen_env, robobin_env] + dmc_envs,
                                  config.action_repeat, config.size, use_goal_idx, log_per_goal)
        env = envs.PadActions(
            env, list(_env.action_space for _env in env.envs))
        env = wrappers.NormalizeActions(env)
...
def create_envs(config, logger):
    ...
    make = functools.partial(make_env, config, logger,
                             train_eps=train_eps, eval_eps=eval_eps)
    train_envs = [make('train', log_per_goal=True) for _ in range(config.envs)]
    eval_envs = [make('eval', use_goal_idx=True, log_per_goal=config.test_log_per_goal)
...
def parse_dreamer_args():
    ...
    parser.add_argument('--configs', default=['defaults', 'lexa_cosine'], nargs='+')
...
def main(logdir, config):
    ...
    eval_envs, eval_eps, train_envs, train_eps, acts = create_envs(
        config, logger)
    prefill = max(0, config.prefill - count_steps(config.traindir))
    print(f'Prefill dataset ({prefill} steps).')
    def random_agent(o, d, s): return ([acts.sample() for _ in d], s)
    ...
    while agent._step.numpy().item() < config.steps:
        ...
        state = tools.simulate(
            agent, train_envs, config.eval_every, state=state)
