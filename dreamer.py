...
class Dreamer(tools.Module):
  ...
  def _train(self, data):
    ...
    metrics.update(mets)
    # action_embed = post['action_embed'] 
    # decision = post['decision'] 
    # start = {item: post[item] for item in post if item in ['mean', 'std', 'stoch', 'deter']}
    start = post
    # del start['cd_deter_mean'], start['cd_deter_std'], start['decision'], start['action_embed']
    ...
    if self._config.expl_behavior != 'greedy':
        mets = self._expl_behavior.train(post, feat, embed, kl)[-1]
