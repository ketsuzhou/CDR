...
# os.environ['MUJOCO_RENDERER'] = 'osmesa'
# os.environ['MUJOCO_GL'] = 'osmesa'
...
class GCDreamer(Dreamer):
    def __init__(self, config, logger, dataset):
        ...
        self.skill_to_use = tf.zeros([0], dtype=self._float)
    ...
    def _policy(self, obs, state, training, reset):
        ...
        if reset.any() and state is not None:
            ...
            if training and self._config.training_goals == 'batch':
                state[0]['image_goal'], state[0]['goal'] = self.sample_replay_goal(
                    obs)
            else:
                state[0]['image_goal'] = tf.cast(
                    obs['image_goal'], self._float) / 255.0 - 0.5
        ...
        return super()._policy(
            obs, state, training, reset, should_expl=self._should_expl_ep.value)
    ...
    def sample_replay_goal(self, obs):
        ...
        random_goal_states = tf.reshape(
            states, (-1,) + tuple(states.shape[2:]))
...
def process_eps_data(eps_data):
    ...
    new_data = {}
    for key in keys:
        new_data[key] = np.array([eps_data[i][key]
                                for i in range(len(eps_data))]).squeeze()
    return new_data
...
def main(logdir, config):
    ...
    pathlib.Path(
        logdir / "distance_func_logs_trained_model").mkdir(parents=True, exist_ok=True)
    ...
    while agent._step.numpy().item() < config.steps:
        ...
        for ep_idx in range(num_eval_eps):
            ...
            for idx in range(num_goals):
                ...
                video = eval_envs[0]._convert(
                    [t['image'] for t in eval_envs[0]._episode])
        ...
        if config.sync_s3:
            os.system('aws s3 sync '+str(logdir) +
                      ' s3://goalexp2021/research_code/goalexp_data/'+str(logdir))
