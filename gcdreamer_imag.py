...
class GCDreamerBehavior(models.ImagBehavior):
    ...
    def __init__(self, config, world_model, stop_grad):
        ...
        self.only_stoch = config.only_stoch
    ...
    def _gc_reward(self, feat, inp_state=None, action=None, obs=None):
        ...
        if self._config.gc_input == 'embed':
            ...
            if self._config.remove_recur_info == True:
                # inp_feat = self._world_model.heads['pred_init_feat_embed'](
                #     inp_feat).mode()
                inp_feat = self._world_model.heads['pred_init_post_stoch'](
                    inp_state['deter']).mode()
                # inp_feat = self._world_model.heads['pred_init_feat_embed'](
                #     inp_state['stoch'])

            if self._config.gc_reward == 'l2':
                if self.only_stoch:
                    inp_feat = self._world_model.dynamics.get_dist(inp_state).mode()
                # goal_feat = tf.vectorized_map(
                #     self._world_model.get_init_post_stoch, goal_embed)
                goal_feat = self._world_model.get_im_embeding(goal_embed)
                # goal_feat = tf.vectorized_map(
                #     self._world_model.get_im_embeding, goal_embed)
                # if self.only_stoch:
                #     goal_feat = tf.split(
                #         goal_feat, [inp_state['stoch'].shape[-1], -1], -1)[0]
                #     # a = tf.split(
                #     #     tf.reshape(goal_feat, [*goal_feat.shape[:-1], 10, -1]), 2, -1)[1]
                #     goal_feat = tf.reshape(tf.split(
                #         tf.reshape(goal_feat, [*goal_feat.shape[:-1], 10, -1]), 2, -1)[1], 
                #         [*goal_feat.shape[:-1], -1])
                    # goal_feat = tf.reshape(tf.split(
                    #     tf.reshape(goal_feat, [*goal_feat.shape[:-1], 10, -1]), 2, -1)[1], 
                ...
                # aa = - inp_feat.log_prob(goal_feat)
                return 0.5-tf.reduce_mean((goal_feat - inp_feat) ** 2, -1)/8
    ...
    def train(
        ...
        with (tape or tf.GradientTape(persistent=self._config.gc_input == 'skills')) as actor_tape:
            ...
            # ind = tf.range(goal.shape[1], delta=2)
            # goal = tf.gather(goal, ind, axis=1) 
            # start = {name: tf.gather(start[name], ind, axis=1) for name in start}
        ...
        if self._config.gc_reward == 'dynamical_distance' and self._config.dd_train_imag:
            with tf.GradientTape() as df_tape:
                ...
                _inp = self._world_model.dynamics.get_dist(imag_state).mode() if 'feat' in self._config.dd_inp \
                    else self._world_model.heads['embed'](imag_feat).mode()    
    ...
    def _imagine(self, start, policy, horizon, repeats=None, goal=None):
        ...
        def step(prev, step):
            ...
            succ = dynamics.img_step(
                state, action, first_state=step == 0,
